...
@section('content')
    <form action="{{route('events::store')}}" method="post" class="pt-3" enctype="multipart/form-data">
        ...
        <input type="hidden" name="thumb_path" value="{{$event->thumbnail_url}}">
        <x-adminlte-input name="title" placeholder="Title" value="{{$event->title}}"/>
        <x-adminlte-textarea name="description" placeholder="Insert description..."
                             rows="5">{{$event->description}}</x-adminlte-textarea>
        <div class="row mb-3">
            <div class="col-3">
            ...
            <div class="col-3">
                <x-adminlte-select name="category_id">
                    ...
                    @foreach($categories as $category)
                        <option value="{{$category->id}}" @selected($event->category_id === $category->id)>{{$category->title}}</option>
                    @endforeach
                </x-adminlte-select>
            </div>
            <div class="col-3">
                <x-adminlte-input name="slots" placeholder="Slots" type="number" value="{{$event->slots}}"/>
            </div>
            <div class="col-3">
                <x-adminlte-input-date name="planing_time" label-class="text-danger" value="{{$event->planing_time}}"
        ...
        <x-events::address-input value="{{$event->address}}"/>

        <select name="tags[]" id="tags" multiple="multiple" class="form-control mb-3">
            @foreach($event->tags as $tag)
                <option value="{{$tag->name}}" @selected($event->tags->contains($tag))>{{$tag->name}}</option>
            @endforeach
        </select>
